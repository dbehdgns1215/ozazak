...
public class EssayProcessingService {
        ...
        public CompletableFuture<EssayGenerationResult> processAsync(
                        ...
                        Long accountId) {
                try {
                        log.info("Processing essay {} in thread {}", essayData.getEssayId(),
                                        Thread.currentThread().getName());

                        // 1. 참조 Block 조회
                        List<AIGenerationPort.ReferenceBlock> referenceBlocks = loadReferenceBlocks(
                                        item.getReferenceBlockIds());
                        ...
                        AIGenerationPort.AIGenerationRequest request = AIGenerationPort.AIGenerationRequest.builder()
                                        ...
                                        .recruitmentUrl(context.getRecruitmentUrl()) // Added URL
                                        ...
                                        .charMax(essayData.getCharMax())
                                        .recruitmentContent(context.getRecruitmentContent()) // 추가
                                        .build();
                ...
                return blockIds.stream()
                                ...
                                .map(block -> AIGenerationPort.ReferenceBlock.builder()
                                                ...
                                                .categories(block.getCategories() != null
                                                                ? BlockCategoryMapper
                                                                                .toNames(block.getCategories().value())
                                                                : List.of())
                                                .build())
                                .collect(Collectors.toList());
        }
                ...
                if (isEmpty) {
                        ...
                        Essay currentEssay = loadEssayPort.findById(essayData.getEssayId())
                                        .orElseThrow(() -> new IllegalArgumentException(
                                                        "Essay not found: " + essayData.getEssayId()));
